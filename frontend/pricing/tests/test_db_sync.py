...
def test_download_db_returns_true_and_calls_s3_on_success(tmp_path: Path):
    ...
    with patch("pricing.db_sync.boto3.client", return_value=mock_s3):
...
def test_download_db_reraises_errors_that_are_not_missing_key(tmp_path: Path):
    """A 404 is a normal "nothing there yet" state. Anything else --
    wrong permissions, a typo'd bucket name -- is a real problem and
    should surface loudly, not be silently swallowed alongside the 404 case.
    """
    mock_s3 = MagicMock()
...
def test_upload_db_refuses_to_upload_a_file_that_does_not_exist(tmp_path: Path):
